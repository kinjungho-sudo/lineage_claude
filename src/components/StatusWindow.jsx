            ...
            <div className="p-2 border-b border-[#333] flex justify-between items-end bg-[#222]">
                <div className="flex flex-col">
                    <span className="text-[#a59c77] font-bold text-sm tracking-wider uppercase">
                        {state.characterClass === 'elf' ? 'ELF' : 'KNIGHT'}
                    </span>
            ...
            <div className="flex-1 bg-[#1a1a1a] p-2 text-xs text-[#aaa]">
                <table className="w-full">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td className="text-right text-white whitespace-nowrap">
                                ...
                                {state.combatState?.magicHelmStrEndTime && Date.now() < state.combatState.magicHelmStrEndTime && (
                                    <span className="text-blue-400 text-[9px] ml-1 font-bold animate-pulse" title="마법 투구 (힘) 효과">[MIGHTY]</span>
                                )}
                                {availablePoints > 0 && (
                            ...
                            <td></td>
                            <td>DEX</td><td className="text-right text-white">{totalDex}</td>
                        </tr>
                        <tr>
                            <td>CON</td>
                            <td className="text-right text-white whitespace-nowrap">
                        ...
                        <tr>
                            <td>WIS</td><td className="text-right text-white">{totalWis}</td>
                            <td></td>
                            <td> 명중</td><td className="text-right text-white text-orange-400 font-bold">{accuracy}</td>
                        </tr>
                        <tr>
                            ...
                            <td className="text-right text-orange-300 font-bold whitespace-nowrap">
                                <div className="flex flex-col items-end">
                                    {(() => {
                                        ...
                                        const maxRandomBonus = stats.weaponEnchant >= 7 ? (40 + (stats.weaponEnchant - 7) * 15) : 0;
                                        
                                        return (
                                            <>
                                                <div className="flex items-center gap-1">
                                                    <span className="text-[9px] text-[#888]">공격력</span>
                                                    <span>{universalAtk} {maxRandomBonus > 0 && <span className="text-[#ffcc00] text-[9px]">(+{maxRandomBonus})</span>}</span>
                                                </div>
                                                <div className="text-[9px] text-[#555] mt-0.5 leading-none">
                                                    기본:{stats.smallAtk}/보너스:{fixedBonus} {maxRandomBonus > 0 ? `/랜덤:max+${maxRandomBonus}` : ''}
                                                </div>
                                            </>
                                        );
                                    })()}
