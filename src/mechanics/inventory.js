...
export const handleUsePotion = (state, potionUid) => {
    ...
    // [New] 마법서(스킬북) 사용 로직
    if (potion.type === 'book' && potion.useData?.type === 'learn_skill') {
        if (state.level < potion.useData.minLevel) {
            return { ...state, logs: [`[시스템] 레벨 ${potion.useData.minLevel} 이상부터 배울 수 있습니다.`, ...state.logs] };
        }
        
        const skillId = potion.useData.skillId;
        const currentSpells = state.learnedSpells || [];
        if (currentSpells.includes(skillId)) {
            return { ...state, logs: [`[시스템] 이미 배운 스킬입니다.`, ...state.logs] };
        }
        ...
        const now = Date.now();
        const newAnnouncements = state.pendingAnnouncements ? [...state.pendingAnnouncements] : [];
        ...
        return {
            ...state,
            inventory: newInventory,
            learnedSpells: [...currentSpells, skillId],
            logs: [`[마법] ${potion.name}을(를) 읽고 스킬을 깨우쳤습니다!`, ...state.logs],
            pendingAnnouncements: newAnnouncements,
            combatState: {
                ...state.combatState,
                levelUpEffect: { timestamp: now } // Reuse levelup effect as "fire pillar"
            }
        };
    }
    ...
    const healAmount = potion.healAmount || 0;
...
export const handleUseScroll = (state, scrollUid, targetUid) => {
    ...
    let newAnnouncements = state.pendingAnnouncements ? [...state.pendingAnnouncements] : [];
    ...
    if (targetIndexNew !== -1) {
        ...
        if (result.result === ENCHANT_RESULTS.SUCCESS) {
            ...
            if (isAnnounce) {
                newAnnouncements.push({
                    type: 'enchant_success',
                    text: `[속보] __USER__님이 +${result.newEnchant} ${targetItem.name} 강화에 성공했습니다!`
                });
            }
        } else if (result.result === ENCHANT_RESULTS.DESTROYED) {
 ...
 */
export const handleDepositWarehouse = (state, itemUid, quantity = 1) => {
    const itemIndex = state.inventory.findIndex(i => i.uid === itemUid);
    if (itemIndex === -1) return state;

    const item = state.inventory[itemIndex];
    if (item.isEquipped) {
        return { ...state, logs: ['[시스템] 장착 중인 아이템은 창고에 맡길 수 없습니다.', ...state.logs] };
    }
    ...
    const isStackable = item.type === 'scroll' || item.type === 'potion';
    if (isStackable) {
        const existingItemIndex = newWarehouse.findIndex(i => i.id === item.id);
        if (existingItemIndex >= 0) {
            newWarehouse[existingItemIndex] = { ...newWarehouse[existingItemIndex], count: (newWarehouse[existingItemIndex].count || 0) + depositCount };
        } else {
            newWarehouse.push({ ...item, uid: Date.now() + Math.random(), count: depositCount });
        }
    } else {
        // 단일 아이템은 그대로 배열에 추가하되, 혹시 한꺼번에 넣을 경우 루프 돌림
        for (let i = 0; i < depositCount; i++) {
            newWarehouse.push({ ...item, uid: Date.now() + Math.random() + i, count: 1 });
        }
    }
    ...
    return {
        ...
        logs: [`[창고] ${item.enchant > 0 ? `+${item.enchant} ` : ''}${item.name} (${depositCount}개) 보관 완료`, ...state.logs]
    };
};

/**
 * 창고 출고 로직 (한 종류당 출고 시 아이템 1종류 단위 출고: 100 아데나 비용)
 * @param {Object} state 
 * @param {number} itemUid 
 * @param {number} quantity 출고 수량
 */
export const handleWithdrawWarehouse = (state, itemUid, quantity = 1) => {
    const WITHDRAW_FEE = 100;
    if (state.adena < WITHDRAW_FEE) {
        return { ...state, logs: ['[시스템] 창고 수수료가 부족합니다. (필요: 100 아데나)', ...state.logs] };
    }

    const itemIndex = state.warehouse.findIndex(i => i.uid === itemUid);
    if (itemIndex === -1) return state;

    const item = state.warehouse[itemIndex];
    ...
    if (item.count > withdrawCount) {
        newWarehouse[itemIndex] = { ...item, count: item.count - withdrawCount };
    } else {
        newWarehouse = newWarehouse.filter(i => i.uid !== itemUid);
    }
    ...
    if (isStackable) {
        ...
        if (existingItemIndex >= 0) {
            newInventory[existingItemIndex] = { ...newInventory[existingItemIndex], count: (newInventory[existingItemIndex].count || 0) + withdrawCount };
        } else {
